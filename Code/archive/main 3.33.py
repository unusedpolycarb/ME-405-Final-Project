...
def task1(shares):
    ...
    #corrective mode = 1-5, 3 is neutral
    corrective_mode = 3
    #correction required = Bool
    ...
    while True:
        ...
        if state == 2:
            ...
            #if all sensors tripped just add to count
            if False not in line_results:
                bar_toggle = True
            else:
                if bar_toggle is True:
                    bar_toggle = False
                    num_of_bars += 1

            #if line is over sensor 1
            if line_results[0] is True and corrective_mode != 1:
                corrective_mode = 1
                correction_required = True
                #increase motor A by a lot
                #mot_A_vel = NOMINAL_WHEEL_VEL + CORRECTIVE_VEL*1.5
                #mot_B_vel = NOMINAL_WHEEL_VEL - CORRECTIVE_VEL*1.5 #rad/s


            #elif line is over sensor 2
            elif line_results[1] is True and corrective_mode != 2:
                corrective_mode = 2
                correction_required = True
                #increase motor A by a little
                #mot_A_vel = NOMINAL_WHEEL_VEL + CORRECTIVE_VEL
                #mot_B_vel = NOMINAL_WHEEL_VEL - CORRECTIVE_VEL #rad/s
                

            #if line is over sensor 5
            elif line_results[4] is True and corrective_mode != 5:
                corrective_mode = 5
                correction_required = True
                #decrease motor A by a good amount
                #mot_A_vel = NOMINAL_WHEEL_VEL - CORRECTIVE_VEL*1.5
                #mot_B_vel = NOMINAL_WHEEL_VEL + CORRECTIVE_VEL*1.5 #rad/s
                
            #elif line is over sensor 4
            elif line_results[3] is True and corrective_mode != 4:
                corrective_mode = 4
                correction_required = True
                #decrease motor A by a lot
                #mot_A_vel = NOMINAL_WHEEL_VEL - CORRECTIVE_VEL
                #mot_B_vel = NOMINAL_WHEEL_VEL + CORRECTIVE_VEL #rad/s

            #elif line is in neutral location
            elif line_results[2] is True and corrective_mode != 3:
                corrective_mode = 3
                correction_required = True
                #mot_A_vel = NOMINAL_WHEEL_VEL
                #mot_B_vel = NOMINAL_WHEEL_VEL 

            else:

                corrective_mode = 6
  
            #depending on which motor were calculated to be different, set the flag properly
        ...
        if state == 9 and flag == RomiFlags.none:
            ...
            my_queue.put(int(YAW_RATE*1000))
            my_queue.put(0)
            flag = None
            state = 10

        if state == 10:
            if True in my_array.get_line() and flag == RomiFlags.none:
                print("found line")
                my_share.put(RomiFlags.soft_disable_motor)
                my_queue.put(2)
...
#Motor B
    ...
    while True:  
        ...
        if mot_A_state == 2 or mot_A_state == 3:
            ...
            #print(f"A speed: {speed_tracker_A}")
            mot_A.set_speed(speed_tracker_A)
        
            
        if mot_B_state == 2 or mot_B_state == 3:
            ...
            #print(f"B speed: {speed_tracker_B}")
        ...
        if alarm_heading != 0:
            ...
            if alarm_heading + 100 > poll_heading and alarm_heading - 100 < poll_heading:
               print("arrived!")
               alarm_heading = 0
               my_share.put(RomiFlags.soft_disable_motor)
               my_queue.put(2)
...
def task3(shares):
    ...
    while True:
        ...
        if switch_A_pressed is True and flag == RomiFlags.none:
            my_share.put(RomiFlags.switch_A)
            flag = None
            switch_A_pressed = False

        if switch_B_pressed is True and flag == RomiFlags.none:
